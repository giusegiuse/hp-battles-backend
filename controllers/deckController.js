...
exports.getOppenentDeckCharacters = catchAsync(async (req, res, next) => {
    ...
    const charactersWithData = characters.map((character) => {
        const deckCharacter = deck.characters.find((char) => String(char.character) === String(character._id));
        return {
            ...character.toObject(),
            life: deckCharacter.current_health,
        };
    });
...
exports.checkCharactersInLife = catchAsync(async (req, res, next) => {
    const {userId} = req.params;
    const deck = await Deck.findOne({playerChallenger: userId}).sort({
        creationDate: -1,
    });
    const charactersInLife = deck.characters.filter((char) => char.current_health > 0);
    if (charactersInLife.length === 0) {
